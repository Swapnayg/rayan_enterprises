...
def goods_dashboard_index(userid): 
    ...
    comm_count_diff = 0
    comm_count_status = 0
    cw_goods_txt = text("SELECT * FROM test.goods_nlc Where userid = "+str(userid)+" AND datetime >= '"+ start_of_week +"' AND datetime <= '"+today+"'") 
    cw_goods = db.session.execute(cw_goods_txt)
    lw_goods_txt = text("SELECT * FROM test.goods_nlc Where userid = "+str(userid)+" AND datetime >= '"+ lw_start_of_week +"' AND datetime <= '"+lw_today+"'") 
    ...
    for ledg_i in ledger_data:
        ...
        if ("GD_" in str(ledg_i.ledger_bill).strip()):
            try:
                inv_data = GoodsNlc.query.filter(and_(GoodsNlc.bilty_no == str(ledg_i.ledger_bill).strip(), GoodsNlc.userid == userid)).one()
