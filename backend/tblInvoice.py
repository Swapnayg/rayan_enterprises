...
class TblInvoice(db.Model):
    ...
    client_name = db.relationship('Clients', backref='TblInvoice.client_id')
    warehouse_name = db.relationship('Warehouse', backref='TblInvoice.warehouse_id')
    ...
    def __init__(self,id,client_id, warehouse_id,client_name,warehouse_name,invoice_num,invoice_refer,invoice_date,invoice_due_date,invoice_tax,invoice_discount,total_tax,total_discount,shipping,grand_total,invoice_items,invoice_note,payment_currency,payment_terms,invoice_status,datetime,userid):
        self.id = id
        self.client_id = client_id
        self.warehouse_id = warehouse_id
        self.client_name = TblInvoice.client_id.client_name
        self.warehouse_name = TblInvoice.warehouse_id.ware_name
    ...
    def map(self):
        return {'id': self.id, 'client_id': self.client_id, 'warehouse_id': self.warehouse_id,'client_name': self.client_name.client_name, 'warehouse_name': self.warehouse_name.ware_name,'invoice_num':self.invoice_num, 'invoice_refer': self.invoice_refer,'invoice_date':self.invoice_date,'invoice_due_date':self.invoice_due_date,  'invoice_tax': self.invoice_tax, 'invoice_discount': self.invoice_discount,'total_tax':self.total_tax,'total_discount':self.total_discount,  'shipping': self.shipping, 'grand_total': self.grand_total,'invoice_items':self.invoice_items,'invoice_note':self.invoice_note,'payment_currency':self.payment_currency,'payment_terms':self.payment_terms,'invoice_status':self.invoice_status,'datetime':self.datetime, "userid":self.userid}
