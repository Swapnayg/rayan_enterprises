...
class PurchItems(db.Model):
    __tablename__ = "purch_items"
    id = db.Column(db.Integer, primary_key = True)
    item_name = db.Column(db.String(500))
    item_product = db.Column(db.Integer,db.ForeignKey('tbl_product.id'))
    item_invoice =db.Column(db.Integer,db.ForeignKey('tbl_order.id'))
    item_name_name = db.relationship('TblProduct', backref='PurchItems.item_product')
    item_invoice_name = db.relationship('TblOrder', backref='PurchItems.item_invoice')    
    ...
    def __init__(self,id,item_name, item_product,item_invoice,item_name_name,item_invoice_name,item_qnty,item_rate,item_per_tax,item_tax,item_disc,item_amount,item_description,datetime,item_disc_val, userid):
        ...
        self.item_name_name = PurchItems.item_product.product_name
        self.item_invoice_name = PurchItems.item_invoice.order_num
    ...
    def map(self):
        return {'id': self.id, 'item_name': self.item_name, 'item_product': self.item_product,'item_invoice': self.item_invoice, 'item_name_name': self.item_name_name.product_name, 'item_invoice_name': self.item_invoice_name.order_num,'item_qnty':self.item_qnty,'item_rate':self.item_rate,  'item_per_tax': self.item_per_tax, 'item_tax': self.item_tax,'item_disc':self.item_disc,'item_amount':self.item_amount,  'item_description': self.item_description, 'datetime': self.datetime, 'item_disc_val':self.item_disc_val, "userid":self.userid}
